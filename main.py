...
print(Fore.GREEN+"Developed and coded by .gg/mcalts community")
#Before using bots please enter your webhook urls in the conifg and please change all emoji.
#Setup your ltc addy
def load_config():
...
@bot.command()
async def checkcpn(ctx): # Command to check coupons available.
...
async def help(ctx):
    ...
    embed.add_field(name=".couponcreate ", value="To create a discount coupon.", inline=True)
...
async def restocknotif(ctx):
    if ctx.author.id not in bot.staff:
        await ctx.send("You do not have permission to use this command.")
        return
    channel_id = config["restockchannel"]  
...
async def restock(ctx,productid):
    ...
    try:
        ...
        if len(message.attachments) > 0:
            ...
            response = requests.put(f'https://api-internal.sellauth.com/v1/shops/{config["shop_id"]}/products/{productid}/deliverables/append',headers=headers,json=json_data)
...
async def bal(ctx):
    ...
    response = requests.get(
        ...
        headers={'Authorization': f'Bearer {config["btoken"]}'},
    )
    data = response.json()
    print(response.text)
